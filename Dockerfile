...
RUN apt-get update && apt-get -y upgrade && \
    ...
    pip3 install --no-cache-dir patacrep && \
    wget -nv -O lilypond.sh 'https://lilypond.org/download/binaries/linux-64/lilypond-2.22.2-1.linux-64.sh' && \
    bash ./lilypond.sh && \
    rsync -av /patch/patacrep/ /usr/local/lib/python3.6/dist-packages/patacrep/ && \
    songbook --version && \
    apt-get autoremove -y && rm -rf /var/cache/apt/archives /var/lib/apt/lists/* && rm -rf /patch ./lilypond.sh
RUN useradd -s /bin/bash -d /scout sgdf \
    && mkdir -p /scout/carnet \
    && chown -R sgdf:sgdf /scout
USER sgdf
ENV LANG='C.UTF-8'
ENV LC_ALL='C.UTF-8'
